...
def _dataset_report(dataset_name: str) -> dict:
    ...
    report = {
        ...
        "escalated_subset_hybrid_metrics": metrics_from_predictions(
            y_true, hybrid_pred
        ),
        "escalated_subset_raw_llm_recommendation_metrics": metrics_from_predictions(
            y_true, raw_llm_pred
        ),
        "escalated_subset_gated_part4_metrics": metrics_from_predictions(
            y_true, final_pred
        ),
...
def main() -> None:
    ...
    for report in reports:
        ...
        print(
            "  escalated-subset F1: "
            f"hybrid={report['escalated_subset_hybrid_metrics']['f1']:.6f}, "
            f"raw_llm={report['escalated_subset_raw_llm_recommendation_metrics']['f1']:.6f}, "
            f"gated_part4={report['escalated_subset_gated_part4_metrics']['f1']:.6f}"
        )
