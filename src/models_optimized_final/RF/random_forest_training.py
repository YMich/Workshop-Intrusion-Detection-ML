        ...
        json.dump(
            {
                ...
                "hyperparameter_scope": "identical_shared_values_for_dataset2_and_dataset3",
            },
            handle,
            indent=2,
        )
...
def run_final_rf_pipeline() -> pd.DataFrame:
    ...
    print("Shared RF hyperparameters for BOTH D2 and D3:")
    ...
    print("=" * 100)
    transfer, transfer_predictions = cross_dataset_evaluation(temporal_records)
    transfer.to_csv(RESULT_ROOT / "final_rf_cross_dataset_summary.csv", index=False)
    transfer_predictions.to_csv(
        RESULT_ROOT / "final_rf_cross_dataset_predictions.csv", index=False
    )
    print(
        transfer[
            [
    ...
    with (ARTIFACT_ROOT / "FINAL_RF_MANIFEST.json").open("w", encoding="utf-8") as handle:
        json.dump(
            {
                "representation": "endpoint_aware_RROLL5_32_features",
                "feature_count": 32,
                ...
                "hyperparameters": FINAL_RF_PARAMS,
                "prediction_threshold": PREDICTION_THRESHOLD,
                "same_hyperparameters_dataset2_dataset3": True,
                "sequence_key": ["SourceFile", "Src IP", "Dst IP"],
                "raw_ip_timestamp_used_as_predictors": False,
                "rolling_window": 5,
