...
def _find_project_root(start: Path) -> Path:
    """Find Workshop_final regardless of experiment subdirectory depth."""
    start = Path(start).resolve()
    for candidate in (start.parent, *start.parents):
        if (candidate / "data" / "ingested").exists() and (candidate / "src" / "preprocessing").exists():
            return candidate
    raise RuntimeError(
        "Could not locate project root containing data/ingested and src/preprocessing."
    )
...
# PROJECT_ROOT is derived dynamically from the repository layout.
DATASET_PATHS = {
...
# Final optimized LSTM outputs live in their own namespace.
FINAL_NAMESPACE = "models_optimized_final"
ARTIFACT_ROOT = PROJECT_ROOT / "artifacts" / FINAL_NAMESPACE / "LSTM"
RESULT_ROOT = PROJECT_ROOT / "results" / FINAL_NAMESPACE / "LSTM"
SHARED_SPLIT_ROOT = PROJECT_ROOT / "artifacts" / "splits"

# The final model uses the frozen shared D2/D3 hyperparameter configuration.
SHARED_HYPERPARAMETER_PATH = ARTIFACT_ROOT / "shared_hyperparameters.json"
...
# Final optimized model: retain the accepted O1 brittle/environment-dependent feature pruning.
#
...
# Final optimized model: retain the accepted O2 malicious-only temporal/rate augmentation.
# Two deterministic synthetic variants are generated per malicious SourceFile:
# one faster/compressed and one slower/expanded. Validation and test are untouched.
AUGMENTATION_RANDOM_STATE = 42
AUGMENTATION_COPIES_PER_MALICIOUS_SOURCE = 2
AUGMENTATION_FAST_RANGE = (0.50, 0.90)
AUGMENTATION_SLOW_RANGE = (1.10, 2.00)
AUGMENTED_SOURCE_MARKER = "__FINAL_TIMEWARP__"
...
# ============================================================
# FINAL FIXED SHARED LSTM CONFIGURATION
# ============================================================

# One frozen model configuration is used for Dataset 2 and Dataset 3.
# No hyperparameter search or sensitivity sweep is performed by this final code.
FINAL_SHARED_PARAMS = {
    "sequence_length": 20,
    "lstm_units": (32,),
    "dense_units": 32,
...
def is_timewarp_timing_feature(name: str) -> bool:
    """Raw time-valued O1 input that should scale with elapsed time."""
    n = _normalized_feature_name(name)
    return (
        n == "flow duration"
        or " iat " in f" {n} "
        or n.startswith("active ")
        or n.startswith("idle ")
    )


def is_timewarp_rate_feature(name: str) -> bool:
    """Raw rate-valued O1 input that should vary inversely with elapsed time."""
    n = _normalized_feature_name(name)
    return (
        n.endswith("/s")
        or " rate avg" in n
...
def resolve_timewarp_features(selected_features: list[str]) -> tuple[list[str], list[str]]:
    timing = [f for f in selected_features if is_timewarp_timing_feature(f)]
    rates = [f for f in selected_features if is_timewarp_rate_feature(f)]
    overlap = sorted(set(timing) & set(rates))
    if overlap:
        raise RuntimeError(f"Time-warp feature classification overlaps: {overlap}")
    if not timing:
        raise RuntimeError("Final augmentation found no timing features to augment.")
    if not rates:
        raise RuntimeError("Final augmentation found no rate features to augment.")
    return timing, rates


def make_timewarped_malicious_training_rows(
    train_df: pd.DataFrame,
    selected_features: list[str],
    random_state: int = AUGMENTATION_RANDOM_STATE,
) -> tuple[pd.DataFrame, pd.DataFrame, list[str], list[str]]:
    """Create raw-space malicious-only time-warp copies for TRAINING.

    A factor s represents elapsed-time scaling:
      timing' = s * timing
      rate'   = rate / s

    The original training data are never modified. SourceFile names are suffixed
    so augmented sequences cannot join the original source during construction.
    """
    timing_features, rate_features = resolve_timewarp_features(selected_features)

    missing = [
        f for f in [*timing_features, *rate_features]
        if f not in train_df.columns
    ]
    if missing:
        raise ValueError(f"Final optimized raw training frame is missing time-warp features: {missing}")

    malicious = train_df[train_df[LABEL_COL].astype(int) == MALICIOUS_LABEL].copy()
    if malicious.empty:
        raise ValueError("Final optimized training requires malicious rows in the training split.")

    rng = np.random.default_rng(int(random_state))
    blocks = []
    manifest_rows = []

    for source_file, source_df in malicious.groupby(SOURCE_FILE_COL, sort=True):
        factors = [
            float(rng.uniform(*AUGMENTATION_FAST_RANGE)),
            float(rng.uniform(*AUGMENTATION_SLOW_RANGE)),
        ]
        if AUGMENTATION_COPIES_PER_MALICIOUS_SOURCE != 2:
            raise RuntimeError(
                "The final augmentation design expects exactly two augmented copies per malicious SourceFile."
            )

        for copy_index, factor in enumerate(factors, start=1):
            augmented = source_df.copy()

            for feature in timing_features:
                values = pd.to_numeric(augmented[feature], errors="coerce")
                augmented[feature] = values * factor
            ...
            augmented_source = (
                f"{source_file}{AUGMENTED_SOURCE_MARKER}{copy_index}_s{factor:.6f}"
            )
            augmented[SOURCE_FILE_COL] = augmented_source

            if FLOW_ID_COL in augmented.columns:
                augmented[FLOW_ID_COL] = (
                    augmented[FLOW_ID_COL].astype(str)
                    + f"{AUGMENTED_SOURCE_MARKER}{copy_index}"
                )
            ...
            manifest_rows.append(
                {
                    ...
                    "AugmentedSourceFile": augmented_source,
                    "CopyIndex": int(copy_index),
                    "TimeScaleFactor": float(factor),
                    "OriginalRows": int(len(source_df)),
                    "TimingFeatures": ";".join(timing_features),
                    "RateFeatures": ";".join(rate_features),
                }
            )

    augmented_df = pd.concat(blocks, ignore_index=True)
    manifest = pd.DataFrame(manifest_rows)
    return augmented_df, manifest, timing_features, rate_features


def build_o2_training_sample_weights(
    y_original: np.ndarray,
    y_augmented: np.ndarray,
    original_class_weights: dict[int, float],
) -> np.ndarray:
    """Preserve O1's total class contribution despite malicious duplication."""
    y_original = np.asarray(y_original, dtype=int)
    y_augmented = np.asarray(y_augmented, dtype=int)

    if len(y_augmented) and not np.all(y_augmented == MALICIOUS_LABEL):
        raise ValueError("Final augmentation must contain malicious sequences only.")

    benign_weight = float(original_class_weights[BENIGN_LABEL])
    malicious_weight = float(original_class_weights[MALICIOUS_LABEL])
    malicious_copy_weight = malicious_weight / (1.0 + AUGMENTATION_COPIES_PER_MALICIOUS_SOURCE)

    original_weights = np.where(
        y_original == BENIGN_LABEL,
        benign_weight,
        malicious_copy_weight,
    ).astype(np.float32)
    augmented_weights = np.full(
        len(y_augmented),
        malicious_copy_weight,
        dtype=np.float32,
    )
    return np.concatenate([original_weights, augmented_weights])
...
def fit_lstm(
    ...
    if class_weights is not None and sample_weight is not None:
        raise ValueError("Provide either class_weights or sample_weight, not both.")
...
def train_final_model(
    ...
    # O2 augmentation is generated in RAW feature space after fitting the
    # preprocessor on the original training split. Thus synthetic rows cannot
    # influence medians or RobustScaler parameters.
    augmented_raw, augmentation_manifest, timing_features, rate_features = (
        make_timewarped_malicious_training_rows(
            train_df=train_df,
            selected_features=selected_features,
            random_state=AUGMENTATION_RANDOM_STATE,
        )
    )
    augmented_selected, augmented_features = transform_and_select(
        preprocessor,
        augmented_raw,
        f"{dataset_name}_lstm_final_augmented_train",
        expected_features=selected_features,
    )
    if augmented_features != selected_features:
        raise RuntimeError("Final augmented feature order differs from original training.")

    sequence_length = int(best_params["sequence_length"])

    X_original, y_original, _ = build_sequences(
        train_selected,
        selected_features,
        sequence_length,
        TRAIN_SEQUENCE_STRIDE,
    )
    X_augmented, y_augmented, _ = build_sequences(
        augmented_selected,
        selected_features,
        sequence_length,
        TRAIN_SEQUENCE_STRIDE,
    ...
    expected_augmented_sequences = (
        int((y_original == MALICIOUS_LABEL).sum())
        * AUGMENTATION_COPIES_PER_MALICIOUS_SOURCE
    )
    if len(y_augmented) != expected_augmented_sequences:
        raise RuntimeError(
            f"{dataset_name}: expected {expected_augmented_sequences} augmented malicious "
            f"sequences, got {len(y_augmented)}."
        )
    ...
    # Derive class weights from the ORIGINAL O1 training sequences only. Then
    # split the malicious class weight equally across original + two synthetic
    # variants, so augmentation adds temporal diversity rather than class bias.
    class_weights = calculate_class_weights(y_original)
    sample_weight = build_o2_training_sample_weights(
    ...
    training_info = {
        ...
        "train_rows_augmented": int(len(augmented_raw)),
        "validation_rows": int(len(validation_df)),
        "train_sequences_total": int(len(y_train)),
        "train_sequences_original": int(len(y_original)),
        "train_sequences_augmented": int(len(y_augmented)),
        "validation_sequences": int(len(y_validation)),
        "train_benign_sequences_original": int((y_original == BENIGN_LABEL).sum()),
        "train_malicious_sequences_original": int((y_original == MALICIOUS_LABEL).sum()),
        "class_weight_benign_from_original": float(class_weights[BENIGN_LABEL]),
        "class_weight_malicious_from_original": float(class_weights[MALICIOUS_LABEL]),
        "effective_malicious_weight_per_variant": float(
            class_weights[MALICIOUS_LABEL] / (1.0 + AUGMENTATION_COPIES_PER_MALICIOUS_SOURCE)
        ),
        "augmentation_random_state": int(AUGMENTATION_RANDOM_STATE),
        "augmentation_copies_per_malicious_source": int(AUGMENTATION_COPIES_PER_MALICIOUS_SOURCE),
        "augmentation_fast_range": list(AUGMENTATION_FAST_RANGE),
        "augmentation_slow_range": list(AUGMENTATION_SLOW_RANGE),
        "augmentation_timing_features": list(timing_features),
        "augmentation_rate_features": list(rate_features),
...
def save_training_outputs(
    ...
    final["augmentation_manifest"].to_csv(
        result_dir / "augmentation_manifest.csv",
        index=False,
    )

    # Compatibility note: the legacy filename best_hyperparameters.json is retained
    # because downstream hybrid code expects it. Its contents are the frozen final
    # configuration; no search is performed by this module.
        ...
        json.dump(
            {
                ...
                "optimization_stage": "final_optimized_O1_feature_pruning_plus_O2_temporal_rate_augmentation",
                ...
                "augmentation_design": (
                    "malicious training SourceFiles only; one compressed and one expanded "
                    "raw-space copy per source; timing multiplied by s and rates divided by s"
                ),
                "augmentation_validation_test_changed": False,
                "augmentation_preprocessor_refit_on_synthetic_data": False,
                ...
                "hyperparameter_configuration": (
                    "frozen final shared D2/D3 configuration; no runtime search"
                ),
                ...
                "imbalance_strategy": (
                    "balanced class weights computed from original training sequences only; "
                    "malicious weight divided equally across original plus two augmented variants"
                ),
                "preprocessing_fit_scope": (
                    "fit on original training split only; augmented rows transformed afterward "
                    "with the frozen training-fitted preprocessor"
                ),
                ...
                "test_used_for_training_or_threshold_calibration": False,
...
def get_final_shared_hyperparameters() -> dict:
    """Return the frozen shared D2/D3 hyperparameters used by the final model."""
    params = deepcopy(FINAL_SHARED_PARAMS)
    ...
    for key, value in expected.items():
        if params.get(key) != value:
            raise RuntimeError(
                f"Final shared hyperparameter mismatch for {key}: "
                f"expected {value!r}, got {params.get(key)!r}."
            )
    return params
...
def train_benchmark_datasets(force_resplit: bool = False) -> None:
    """Train the final optimized LSTM for Dataset 2 + Dataset 3."""
    print("\n" + "=" * 90)
    print("LSTM FINAL OPTIMIZED - O1 FEATURE PRUNING + O2 TEMPORAL/RATE AUGMENTATION")
    print("=" * 90)
    print(describe_compute_device())
    print("Final feature pruning. Mandatory dropped features:")
    for feature in MANDATORY_BRITTLE_FEATURES_TO_DROP:
        print(f"  - {feature}")
    print(
    ...
    print(
        "Final temporal augmentation: malicious TRAINING SourceFiles only; "
        f"{AUGMENTATION_COPIES_PER_MALICIOUS_SOURCE} synthetic variants/source; "
        f"fast s in {AUGMENTATION_FAST_RANGE}, slow s in {AUGMENTATION_SLOW_RANGE}."
    )
    print(
        "Time-warp rule: timing *= s, rate *= 1/s. "
        "Validation/test and training-fitted preprocessing remain untouched."
    )
    ...
    shared_best_params = get_final_shared_hyperparameters()

    ARTIFACT_ROOT.mkdir(parents=True, exist_ok=True)
    with SHARED_HYPERPARAMETER_PATH.open("w", encoding="utf-8") as handle:
        json.dump(_jsonable_params(shared_best_params), handle, indent=2)

    print("\nFrozen FINAL SHARED LSTM hyperparameters:")
    ...
    print(f"  saved artifact: {SHARED_HYPERPARAMETER_PATH}")
    ...
    for dataset_name in BENCHMARK_DATASETS:
        ...
        print("\n" + "-" * 90)
        print(f"FINAL OPTIMIZED MODEL - {dataset_name.upper()}")
        print("-" * 90)
        ...
        if selected_schema is None:
            ...
            raise RuntimeError(
                f"{dataset_name}: Final selected feature schema differs across datasets."
            )

        print(f"Final feature count: {len(final['selected_features'])}")
        print(
            "Training sequences | "
            f"original={final['training_info']['train_sequences_original']:,}, "
            f"augmented={final['training_info']['train_sequences_augmented']:,}, "
            f"total={final['training_info']['train_sequences_total']:,}"
        )
        print(
