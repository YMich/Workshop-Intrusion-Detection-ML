...
# Baseline and accepted-O1 locations are read-only inputs for Optimization 2.
...
# Keep Optimization-2 outputs separate so baseline and accepted O1 remain reproducible.
ACCEPTED_O1_NAME = "optimization_1_brittle_feature_removal"
ACCEPTED_O1_ARTIFACT_ROOT = BASELINE_ARTIFACT_ROOT / ACCEPTED_O1_NAME
ACCEPTED_O1_RESULT_ROOT = BASELINE_RESULT_ROOT / ACCEPTED_O1_NAME
OPTIMIZATION_NAME = "optimization_2_temporal_augmentation"
...
# O2 reuses the exact shared hyperparameters selected for the baseline.
...
# Optimization 2: remove features identified as brittle/environment-dependent.
#
...
# Optimization 2: time-warp malicious TRAINING traffic only.
...
AUGMENTED_SOURCE_MARKER = "__O2_TIMEWARP__"
...
def resolve_timewarp_features(selected_features: list[str]) -> tuple[list[str], list[str]]:
    ...
    if not timing:
        raise RuntimeError("O2 found no timing features to augment.")
    if not rates:
        raise RuntimeError("O2 found no rate features to augment.")
    return timing, rates
...
def make_timewarped_malicious_training_rows(
    ...
    if missing:
        raise ValueError(f"O2 raw training frame is missing time-warp features: {missing}")

    malicious = train_df[train_df[LABEL_COL].astype(int) == MALICIOUS_LABEL].copy()
    if malicious.empty:
        raise ValueError("O2 requires malicious rows in the training split.")
    ...
    for source_file, source_df in malicious.groupby(SOURCE_FILE_COL, sort=True):
        ...
        if AUGMENTATION_COPIES_PER_MALICIOUS_SOURCE != 2:
            raise RuntimeError(
                "Current O2 design expects exactly two augmented copies per malicious SourceFile."
            )
...
def build_o2_training_sample_weights(
    ...
    if len(y_augmented) and not np.all(y_augmented == MALICIOUS_LABEL):
        raise ValueError("O2 augmentation must contain malicious sequences only.")
...
def train_final_model(
    ...
    augmented_selected, augmented_features = transform_and_select(
        preprocessor,
        augmented_raw,
        f"{dataset_name}_lstm_o2_augmented_train",
        expected_features=selected_features,
    )
    if augmented_features != selected_features:
        raise RuntimeError("O2 augmented feature order differs from original training.")
        ...
        json.dump(
            {
                ...
                "optimization_stage": "O2_temporal_rate_augmentation_on_accepted_O1",
                ...
                "accepted_o1_result_root": str(ACCEPTED_O1_RESULT_ROOT),
                "augmentation_design": (
                ...
                "imbalance_strategy": (
                    "O1 balanced class weights computed from original training sequences only; "
...
def load_baseline_shared_hyperparameters() -> dict:
    ...
    if not SHARED_HYPERPARAMETER_PATH.exists():
        raise FileNotFoundError(
            "Optimization 2 requires the baseline shared hyperparameter artifact: "
...
def train_benchmark_datasets(force_resplit: bool = False) -> None:
    """Run O2 time-warp augmentation on top of accepted O1 for Dataset 2 + 3."""
    print("\n" + "=" * 90)
    print("LSTM OPTIMIZATION 2 - TEMPORAL/RATE AUGMENTATION")
    print("=" * 90)
    print(describe_compute_device())
    print("Accepted O1 feature pruning remains active. Mandatory dropped features:")
    for feature in MANDATORY_BRITTLE_FEATURES_TO_DROP:
        print(f"  - {feature}")
    print(
    ...
    print(
        "O2 augmentation: malicious TRAINING SourceFiles only; "
    ...
    for dataset_name in BENCHMARK_DATASETS:
        ...
        print("\n" + "-" * 90)
        print(f"O2 FINAL MODEL - {dataset_name.upper()}")
        print("-" * 90)
        ...
        if selected_schema is None:
            ...
            raise RuntimeError(
                f"{dataset_name}: O2/O1 selected feature schema differs across datasets."
            )

        print(f"O2 feature count (same as accepted O1): {len(final['selected_features'])}")
        print(
            "Training sequences | "
