...
def evaluate_dataset(dataset_name: str) -> dict:
    ...
    selected_test, actual_features = transform_and_select(
        ...
        df=test_df,
        dataset_name=f"{dataset_name}_lstm_o2_test",
        expected_features=selected_features,
    )

    if actual_features != selected_features:
        raise ValueError(
            f"{dataset_name}: O2 test feature schema differs from training."
        )
...
def evaluate_all_datasets(
    ...
    # Section 2.2 evidence: compare O2 with the accepted 43-feature O1 first,
    # and also retain a direct comparison with the original baseline.
    comparison_metrics = [
        "fp", "fn", "tp", "tn", "recall_tpr", "precision", "fpr",
        "f1", "f2", "roc_auc", "pr_auc", "mcc",
    ]

    def _build_comparison(reference_root: Path, reference_label: str, output_name: str):
        comparison_rows = []
        for _, o2_row in summary.iterrows():
            dataset_name = str(o2_row["Dataset"])
            reference_path = reference_root / dataset_name / "test_metrics.csv"
            if not reference_path.exists():
                print(
                    f"[WARN] {reference_label} metrics not found for {dataset_name}: "
                    f"{reference_path}; skipping automatic comparison."
                )
                continue

            reference_row = pd.read_csv(reference_path).iloc[0]
            record = {
                "Dataset": dataset_name,
                "Optimization": "O2_temporal_rate_augmentation",
                "Reference": reference_label,
            }
            for metric in comparison_metrics:
                if metric in reference_row.index and metric in o2_row.index:
                    reference_value = float(reference_row[metric])
                    o2_value = float(o2_row[metric])
                    record[f"{reference_label}_{metric}"] = reference_value
                    record[f"O2_{metric}"] = o2_value
                    record[f"Delta_{metric}"] = o2_value - reference_value
