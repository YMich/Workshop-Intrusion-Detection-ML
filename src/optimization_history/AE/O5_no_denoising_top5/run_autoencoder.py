...
def parse_args():
    parser = argparse.ArgumentParser(
        description=(
            "Run Autoencoder Optimization 5 (remove denoising) using "
...
def main():
    ...
    print("=" * 90)
    print("AUTOENCODER O5 - NO DENOISING")
    print("=" * 90)
    print(
    ...
    print(
        "O5 rule: retain the O3 configuration and change denoising_noise_std "
        "from 0.02 to 0.0 only; keep bottleneck=4, LR=0.002, MSE loss, "
        "L2=1e-5, batch=256 and Top-5 MSE score fixed; recalibrate threshold "
        "on O5 validation only."
    )

    if not args.evaluation_only:
        print("\nPHASE 1 - O5 TRAINING / VALIDATION / THRESHOLD CALIBRATION")
    ...
    print("\nPHASE 2 - O5 FINAL TEST EVALUATION")
