...
OPTIMIZATION_NAME = "optimization_5_no_denoising"
ARTIFACT_ROOT = (
...
# ============================================================
# FROZEN SHARED CONFIGURATION FOR CONTROLLED O5
# ============================================================

# Agreed shared Autoencoder configuration for Dataset 2 and Dataset 3.
# O5 retains O3 and changes ONLY denoising noise std 0.02 -> 0.0.
SHARED_FIXED_PARAMS = {
    "encoder_hidden_dims": (64, 32),
    "bottleneck_dim": 4,
    "hidden_activation": "relu",
        ...
        json.dump(
            {
                ...
                "hyperparameter_selection_scope": (
                    "O3 configuration retained; O5 changes denoising noise std only (0.02 -> 0.0)"
                ),
                "threshold_calibration_scope": "O5 validation split only",
                "optimization_stage": "O5_no_denoising",
                "shared_configuration": "64-32_b4_lr0.002_mse_l2_1e-5_noise0.02_batch256_top5_mse",
...
def train_dataset(
    ...
    """
    print("\\n" + "=" * 90)
    print(f"AUTOENCODER O5 - NO DENOISING - {dataset_name.upper()}")
    print("=" * 90)
    ...
    o5_params, o5_score_mode = load_shared_fixed_configuration()

    print("\nO5 controlled experiment - O3 retained, denoising only changed noise std 0.02 -> 0.0:")
    for key, value in o5_params.items():
        print(f"  {key}: {value}")
    print(f"Frozen anomaly score mode: {o5_score_mode}")
    print("O5 change: denoising_noise_std 0.02 -> 0.0 only; O1 pruning, bottleneck=4, MSE loss, Top-5 MSE score, and every other setting remain frozen.")

    benign_train_df = benign_only(train_df)
    schema_preprocessor = DatasetPreprocessor(f"{dataset_name}_o5_schema")
    schema_preprocessor.fit(benign_train_df)
    _tmp_df, _tmp_X, _tmp_y, o1_features = transform_and_select(
        schema_preprocessor,
        benign_train_df,
        dataset_name=f"{dataset_name}_o5_schema",
        expected_features=None,
    )

    print(f"O5 feature count (same O1-pruned schema): {len(o1_features)}")
    ...
    final = train_final_model(
        ...
        best_params=o5_params,
        score_mode=o5_score_mode,
...
def train_all_datasets(
    ...
    print(
        ...
        "64-32, bottleneck=4, LR=0.002, MSE, L2=1e-5, noise=0.0, "
