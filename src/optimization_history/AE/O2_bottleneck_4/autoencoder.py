...
O1_RESULT_ROOT = (
    ...
    / "optimization_1_brittle_feature_removal"
)
OPTIMIZATION_NAME = "optimization_2_bottleneck_4"
ARTIFACT_ROOT = (
...
# ============================================================
# O1 FEATURE PRUNING RETAINED FOR OPTIMIZATION 2
# ============================================================
...
def transform_and_select(
    ...
    if expected_features is not None and list(selected_features) != list(expected_features):
        ...
        raise ValueError(
            f"{dataset_name}: O2/O1-pruned feature schema differs from training. "
...
# ============================================================
# FROZEN SHARED CONFIGURATION FOR CONTROLLED O2
# ============================================================

# Agreed shared Autoencoder configuration for Dataset 2 and Dataset 3.
# O2 changes ONLY bottleneck_dim from 8 to 4; every other value remains frozen.
SHARED_FIXED_PARAMS = {
    "encoder_hidden_dims": (64, 32),
    "bottleneck_dim": 4,
    "hidden_activation": "relu",
        ...
        json.dump(
            {
                ...
                "hyperparameter_selection_scope": (
                    "O1 configuration retained; O2 changes bottleneck_dim only (8 -> 4)"
                ),
                "threshold_calibration_scope": "O2 validation split only",
                "optimization_stage": "O2_bottleneck_4",
                "shared_configuration": "64-32_b4_lr0.002_huber_l2_1e-5_noise0.02_batch256_top5_mse",
...
def train_dataset(
    ...
    """
    print("\\n" + "=" * 90)
    print(f"AUTOENCODER O2 - BOTTLENECK CONSTRICTION - {dataset_name.upper()}")
    print("=" * 90)
    ...
    o2_params, o2_score_mode = load_shared_fixed_configuration()

    print("\nO2 controlled experiment - O1 retained, bottleneck only changed 8 -> 4:")
    for key, value in o2_params.items():
        print(f"  {key}: {value}")
    print(f"Frozen anomaly score mode: {o2_score_mode}")
    print("O2 change: bottleneck_dim 8 -> 4 only; O1 feature pruning and every other model setting remain frozen.")

    benign_train_df = benign_only(train_df)
    schema_preprocessor = DatasetPreprocessor(f"{dataset_name}_o2_schema")
    schema_preprocessor.fit(benign_train_df)
    _tmp_df, _tmp_X, _tmp_y, o1_features = transform_and_select(
        schema_preprocessor,
        benign_train_df,
        dataset_name=f"{dataset_name}_o2_schema",
        expected_features=None,
    )

    print(f"O2 feature count (same O1-pruned schema): {len(o1_features)}")
    print("Retained O1 mandatory drops:")
    ...
    final = train_final_model(
        ...
        best_params=o2_params,
        score_mode=o2_score_mode,
...
def train_all_datasets(
    ...
    print(
        ...
        "64-32, bottleneck=4, LR=0.002, Huber, L2=1e-5, noise=0.02, "
