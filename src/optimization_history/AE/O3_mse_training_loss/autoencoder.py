...
O2_RESULT_ROOT = (
    ...
    / "optimization_2_bottleneck_4"
)
OPTIMIZATION_NAME = "optimization_3_mse_loss"
ARTIFACT_ROOT = (
...
# ============================================================
# FROZEN SHARED CONFIGURATION FOR CONTROLLED O3
# ============================================================

# Agreed shared Autoencoder configuration for Dataset 2 and Dataset 3.
# O3 retains the O2 bottleneck=4 and changes ONLY training loss Huber -> MSE.
SHARED_FIXED_PARAMS = {
    "encoder_hidden_dims": (64, 32),
    "bottleneck_dim": 4,
    "hidden_activation": "relu",
        ...
        json.dump(
            {
                ...
                "hyperparameter_selection_scope": (
                    "O2 configuration retained; O3 changes training loss only (Huber -> MSE)"
                ),
                "threshold_calibration_scope": "O3 validation split only",
                "optimization_stage": "O3_mse_loss",
...
def train_dataset(
    ...
    """
    print("\\n" + "=" * 90)
    print(f"AUTOENCODER O3 - MSE TRAINING LOSS - {dataset_name.upper()}")
    print("=" * 90)
    ...
    o3_params, o3_score_mode = load_shared_fixed_configuration()

    print("\nO3 controlled experiment - O2 retained, training loss only changed Huber -> MSE:")
    for key, value in o3_params.items():
        print(f"  {key}: {value}")
    print(f"Frozen anomaly score mode: {o3_score_mode}")
    print("O3 change: training loss Huber -> MSE only; O1 pruning, bottleneck=4, and every other setting remain frozen.")

    benign_train_df = benign_only(train_df)
    schema_preprocessor = DatasetPreprocessor(f"{dataset_name}_o3_schema")
    schema_preprocessor.fit(benign_train_df)
    _tmp_df, _tmp_X, _tmp_y, o1_features = transform_and_select(
        schema_preprocessor,
        benign_train_df,
        dataset_name=f"{dataset_name}_o3_schema",
        expected_features=None,
    )

    print(f"O3 feature count (same O1-pruned schema): {len(o1_features)}")
    ...
    final = train_final_model(
        ...
        best_params=o3_params,
        score_mode=o3_score_mode,
...
def train_all_datasets(
    ...
    print(
        ...
        "64-32, bottleneck=4, LR=0.002, MSE, L2=1e-5, noise=0.02, "
