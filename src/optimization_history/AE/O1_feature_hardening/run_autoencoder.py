...
def parse_args():
    parser = argparse.ArgumentParser(
        description=(
            "Run Autoencoder Optimization 1 (brittle feature removal) using "
...
def main():
    ...
    print("=" * 90)
    print("AUTOENCODER O1 - BRITTLE FEATURE REMOVAL")
    print("=" * 90)
    print(
    ...
    print(
        "O1 rule: remove Init Win Bytes, Total Connection Flow Time, "
        "Fwd/Bwd Header Length, and all *Flag* features except URG Flag Count."
    )
    print(
        "Model rule: reuse baseline AE architecture/loss/score mode; "
        "recalibrate threshold on O1 validation only."
    )
