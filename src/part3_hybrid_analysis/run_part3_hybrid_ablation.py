...
"""
Part 3 hybrid analysis / ablation tests:
    1) LSTM baseline
    2) LITEMV baseline
...
Design goals
------------
- Use only the exact frozen final implementations under src/models_optimized_final:
...
- Consume the recovered report-facing Part-3 LSTM/LITEMV validation/test evidence directly;
...
- Calibrate hybrid routing on VALIDATION only.
...
This script does not perform model hyperparameter search or model optimization.
Model implementations/configurations are frozen in src/models_optimized_final; fitted Part-3 weights are isolated. Only hybrid
routing thresholds and architecture choice are calibrated on VALIDATION.
"""
...
if getattr(lstm, "FINAL_NAMESPACE", None) != "models_optimized_final":
    raise RuntimeError(
        "Loaded LSTM does not declare FINAL_NAMESPACE='models_optimized_final'."
    )
...
# Part 3 uses the recovered fitted snapshot that generated the report-facing
# cascade. Implementations still come from src/models_optimized_final, while
# fitted neural-model instances are isolated from independently retrained Part 2.
PART3_BASE_MODEL_ROOT = (
    PROJECT_ROOT / "artifacts" / "hybrid_pipeline" / "base_models"
)
PART3_HYBRID_RESULT_ROOT = (
    PROJECT_ROOT / "results" / "hybrid_pipeline"
)

RESULT_ROOT = (
    PROJECT_ROOT
    / "results"
    / "part3_hybrid_analysis"
    / "final_lstm_litemv_rf"
)
ARTIFACT_ROOT = (
    PROJECT_ROOT
    / "artifacts"
...
def _load_part3_component_table(
    dataset_name: str,
    split_name: str,
    ...
    if not path.is_file():
        raise FileNotFoundError(
            f"Missing frozen Part-3 {model_label} evidence: {path}"
        )
    ...
    if missing:
        raise RuntimeError(
            f"Frozen Part-3 {model_label} {split_name} evidence is missing "
            f"required columns: {missing}\nFile: {path}"
        )
    ...
    out[LABEL_COL] = pd.to_numeric(out[LABEL_COL], errors="raise").astype(int)
    out[ROW_KEY_COL] = pd.to_numeric(
        out[ROW_KEY_COL], errors="raise"
    ).astype(int)
    out[probability_column] = pd.to_numeric(
        out[probability_column], errors="raise"
    ).astype(float)

    if out[[SOURCE_FILE_COL, ROW_KEY_COL]].duplicated().any():
        raise RuntimeError(
            f"Frozen Part-3 {model_label} {split_name} evidence contains "
...
def load_lstm_prediction_evidence(
    ...
    rule = _load_decision_rule(
        PART3_BASE_MODEL_ROOT / dataset_name,
        "LSTM",
        "LSTM",
    )
    threshold = float(rule["threshold"])
    validation = _load_part3_component_table(
        dataset_name,
        "validation",
        "LSTM_Probability",
        "LSTM",
    )
    test = _load_part3_component_table(
        dataset_name,
        "test",
        "LSTM_Probability",
        "LSTM",
...
def verify_final_lstm_consistency(
    ...
    path = PART3_HYBRID_RESULT_ROOT / dataset_name / "test_metrics.csv"
    if path.is_file():
        table = pd.read_csv(path)
        rows = table.loc[table["Model"].astype(str) == "LSTM"]
        if len(rows) == 1:
            reported = rows.iloc[0].to_dict()
            for key in ("tn", "fp", "fn", "tp"):
                if key in reported and not pd.isna(reported[key]):
                    if reproduced[key] != int(reported[key]):
                        raise RuntimeError(
                            f"{dataset_name}: frozen Part-3 LSTM {key} mismatch: "
                            f"reproduced={reproduced[key]}, "
                            f"reported={int(reported[key])}"
                        )

    print(
        f"[{dataset_name}] frozen Part-3 LSTM evidence verified | "
...
def verify_final_litemv_consistency(
    ...
    reproduced = metrics_from_predictions(y, pred)
    print(
        f"[{dataset_name}] frozen Part-3 LITEMV evidence verified | "
        f"F1={reproduced['f1']:.6f} | FP={reproduced['fp']} | "
        f"FN={reproduced['fn']}"
    )
    return reproduced
...
def evaluate_two_model_routing_candidates(
    ...
    safe = candidates[
        (candidates["fpr"] <= baseline_metrics["fpr"] + EPS)
        & (candidates["fnr"] <= baseline_metrics["fnr"] + EPS)
    ].copy()
    strict = safe[
        candidates.loc[safe.index, "f1"]
    ...
    return candidates, selected, baseline_metrics
...
def evaluate_three_model_routing_candidates(
    validation: pd.DataFrame,
    lstm_threshold: float,
...
def evaluate_dataset(
    ...
    # -------------------------------------------------------------------------
    # VALIDATION-ONLY routing calibration.
    # -------------------------------------------------------------------------
    (
        routing_candidates_lstm_rf,
        selected_lstm_rf,
        _,
    ) = evaluate_two_model_routing_candidates(
    ...
    routing_candidates_lstm_rf.to_csv(
        result_dir
        / "validation_routing_candidates_lstm_rf.csv",
        index=False,
    )

    (
        routing_candidates_lstm_litemv,
    ...
    routing_candidates_lstm_litemv.to_csv(
        result_dir
        / "validation_routing_candidates_lstm_litemv.csv",
        index=False,
    )

    (
        routing_candidates_triple,
        selected_triple,
        _,
    ) = evaluate_three_model_routing_candidates(
        validation=validation,
        lstm_threshold=lstm_threshold,
        litemv_threshold=litemv_threshold,
        rf_threshold=rf_threshold,
    )
    routing_candidates_triple.to_csv(
        result_dir
        / "validation_routing_candidates_lstm_litemv_rf.csv",
        index=False,
    )
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "Default: Dataset 2 and Dataset 3."
        ),
    )
    ...
    methodology = {
        ...
        "model_configuration_source": (
            "frozen final implementations plus recovered Part-3 fitted snapshot"
        ),
        "hybrid_rule_calibration": (
            "VALIDATION only"
        ),
    ...
    print("\n" + "=" * 112)
    print("PART 3 HYBRID ANALYSIS / ABLATION SUMMARY")
    print("=" * 112)
