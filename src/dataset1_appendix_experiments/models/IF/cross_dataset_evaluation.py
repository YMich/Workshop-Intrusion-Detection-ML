...
APPENDIX_ARTIFACT_ROOT = PROJECT_ROOT / "artifacts" / "dataset1_appendix" / "models" / "IF"
FINAL_ARTIFACT_ROOT = PROJECT_ROOT / "artifacts" / "models_optimized_final" / "IF"
CROSS_RESULT_ROOT = PROJECT_ROOT / "results" / "dataset1_appendix" / "cross_dataset" / "IF"
...
def _load_or_rebuild_source_preprocessor(source_dataset: str, source_artifact: Path) -> DatasetPreprocessor:
    """IF preprocessing was fitted on benign SOURCE TRAIN only; reproduce that exactly on pickle mismatch."""
    ...
    try:
        ...
        print(f"[PREPROCESSOR FALLBACK] Rebuilding {source_dataset} IF preprocessing from benign SOURCE TRAIN only.")
        ...
        if benign_train.empty:
            raise RuntimeError(f"{source_dataset}: no benign source-training rows available for IF preprocessing rebuild.")
        preprocessor = DatasetPreprocessor(f"{source_dataset}_if_cross_rebuilt")
...
def evaluate_transfer(source_dataset: str, target_dataset: str) -> dict:
    ...
    print(
        f"IF {source_dataset} -> {target_dataset}: "
