...
def discover_project_root() -> Path:
    """Find Workshop_final by locating the Dataset-1 appendix hybrid copy."""
    start = Path(__file__).resolve().parent
    for candidate in (start, *start.parents):
        hybrid = (
            candidate / "src" / "dataset1_appendix_experiments"
            / "part3_hybrid" / "hybrid_pipeline.py"
        )
        if hybrid.is_file():
            return candidate
    raise RuntimeError(
        "Could not locate project root containing "
        "src/dataset1_appendix_experiments/part3_hybrid/hybrid_pipeline.py."
    )


PROJECT_ROOT = discover_project_root()
PART4_DIR = Path(__file__).resolve().parent
HYBRID_DIR = PROJECT_ROOT / "src" / "dataset1_appendix_experiments" / "part3_hybrid"
...
# Optional development-only expansion. Keep False for the conservative final
# configuration unless validation-only experiments justify changing it.
