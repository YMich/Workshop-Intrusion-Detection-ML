...
def save_plots(
    ...
    ax.set_title("Isolation Forest - Shared Test Confusion Matrix")
    fig.tight_layout()
    ...
    ax.set_title("Isolation Forest - Shared Test ROC")
    fig.tight_layout()
    ...
    ax.set_title("Isolation Forest - Shared Test Precision-Recall")
    fig.tight_layout()
    ...
    ax.axvline(
        threshold,
        linestyle="--",
        label="Validation max-F1 threshold",
    )
    ax.set_xlabel("Anomaly Score")
    ax.set_ylabel("Flow Count")
    ax.set_title("Isolation Forest - Shared Test Anomaly Scores")
    ax.legend()
    fig.tight_layout()
...
def evaluate_dataset(dataset_name: str) -> dict:
    print("\n" + "=" * 90)
    print(
        "ISOLATION FOREST SHARED-SPLIT EVALUATION - "
    ...
    if local_params != shared_params:
        raise RuntimeError(
            f"{dataset_name}: per-dataset Isolation Forest configuration do not "
            "match the shared D2/D3 artifact."
        )
    ...
    (
        ...
        dataset_name=f"{dataset_name}_if_shared_test",
        df=normalized_test,
    ...
    X_test = selected_test[selected_features]
    y_test = selected_test[LABEL_COL].astype(int)
    ...
    prediction_table["Decision_Threshold"] = threshold
    ...
    (
        prediction_table.groupby(LABEL_COL)["Anomaly_Score"]
        .agg(["count", "mean", "std", "median", "min", "max"])
        .reset_index()
        .to_csv(
            result_dir / "anomaly_score_summary_by_class.csv",
            index=False,
        )
    )
    ...
    print(f"Threshold learned on validation: {threshold:.8f}")
...
def evaluate_all_datasets(
    ...
    summary.to_csv(
        RESULT_ROOT / "isolation_forest_shared_test_summary.csv",
        index=False,
    )
    return summary
