...
from isolation_forest import DATASET_PATHS, train_all_datasets
from isolation_forest_evaluation import evaluate_all_datasets


def parse_args():
    parser = argparse.ArgumentParser(
        description=(
            "Run the Dataset-2/Dataset-3 Isolation Forest experiment using one "
            "shared hyperparameter configuration and the persisted source-aware "
            "70/15/15 split."
        )
    )
...
def main():
    ...
    print("=" * 90)
    print("ISOLATION FOREST - D2/D3 SHARED-HYPERPARAMETER HOLDOUT")
    print("=" * 90)
    print(
    ...
    print(
        "Configuration rule: fixed shared baseline; n_estimators=200 and "
        "max_samples=512, with no hyperparameter search during execution."
    )
    print(
        "One-class rule: preprocessing and Isolation Forest are fit on benign "
        "training rows only; validation remains mixed."
    )
    print(
        "Threshold rule: max-F1 threshold is calibrated separately per dataset "
        "on validation only; test remains untouched."
    )
