...
"""
LITEMV baseline for the Cobalt Strike HTTPS C2 project.
...
- Uses the fixed model-agnostic 57-feature schema.
...
- No LSTM-specific feature-pruning or temporal augmentation is applied in this baseline.
...
ARTIFACT_ROOT = PROJECT_ROOT / "artifacts" / "models_baseline" / "LITEMV"
RESULT_ROOT = PROJECT_ROOT / "results" / "models_baseline" / "LITEMV"
SHARED_SPLIT_ROOT = PROJECT_ROOT / "artifacts" / "splits"

# Keep the baseline implementation self-contained: this directory includes its
# own source_aware_split.py. Shared preprocessing/feature-engineering modules
# are loaded from the repository-level src tree.
for _path in [
    MODEL_DIR,
...
# Step-3 baseline schema in the project.
...
# =============================================================================
# FIXED SHARED LITEMV CONFIGURATION
# =============================================================================
...
def transform_and_select(
    ...
    selected_df, selected_features, dropped, manifest = select_and_engineer_dataset(
        dataset_name=dataset_name,
        df=normalized,
        expected_selected_features=expected_features,
    )

    return selected_df, selected_features, dropped, manifest
...
def train_dataset(
    ...
    if len(selected_features) != EXPECTED_FEATURE_COUNT:
        raise ValueError(
            f"{dataset_name}: expected {EXPECTED_FEATURE_COUNT} baseline features, "
